    ...
    public partial class CarBuyingWindow : Form
    {
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (checkBox1.Checked)
            {
                ...
                if (!textBoxPrice.Text.Contains("Кондиціонер: +20000"))
                {
                    textBoxPrice.Text += Environment.NewLine + "Кондиціонер: +20000";
                }
            }
            else
            {
                ...
                textBoxPrice.Text = textBoxPrice.Text.Replace(Environment.NewLine + "Кондиціонер: +20000", "").Trim();
            }
            PriceTotal.Text = $"Кінцева ціна(з ПДВ 20%): {PriceWithVAT.ToString()}";
        }
        ...
        private void comboBoxColorCar_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            // Проверяем, был ли уже добавлен дополнительный сбор для цвета
            if (comboBoxColorCar.SelectedIndex != 0 && !textBoxPrice.Text.Contains("Зміна кольору: +24000"))
            {
                PriceWithVAT += additionalPrice;
                textBoxPrice.Text += Environment.NewLine + "Зміна кольору: +24000";
            }
            else if (comboBoxColorCar.SelectedIndex == 0 && textBoxPrice.Text.Contains("Зміна кольору: +24000"))
            {
                PriceWithVAT -= additionalPrice;
